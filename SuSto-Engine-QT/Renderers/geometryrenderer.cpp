...
void GeometryRenderer::Render(Camera3D* camera, const float4x4 &view, const float4x4 &projection, RenderTarget* target)
{
        ...
        if(mesh_renderer != nullptr)
        {
            ...
            if(curr_mesh != nullptr)
            {
                if(curr_mesh->GetLoaded())
                {
                    ...
                    RenderManager::Instance()->SetUniformInt(program->GetID(), "hasSpecular", has_specular);
